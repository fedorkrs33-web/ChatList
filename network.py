...
class Network:
    ...
    def _send_openai_compatible(model: Model, prompt: str) -> str:
        ...
        try:
            ...
            # Для OpenRouter добавляем HTTP-Referer (рекомендуется)
            if model.provider == "openrouter":
                headers["HTTP-Referer"] = "https://github.com/yourusername/chatlist"  # Опционально
                headers["X-Title"] = "ChatList"  # Опционально

            # Определяем модель (можно улучшить: хранить в БД)
            if model.provider == "openrouter":
                # Для OpenRouter используем популярную модель по умолчанию
                model_name = "openai/gpt-3.5-turbo"
            elif "gpt" in model.name.lower():
                model_name = "gpt-4"
            else:
